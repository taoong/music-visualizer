/**
 * Core type definitions for Music Visualizer
 */

// Audio analysis modes
export type AnalysisMode = 'freq' | 'mic' | 'interactive';
export type VizMode = 'circle' | 'spectrum' | 'tunnel' | 'tetris' | 'lasers' | 'text' | 'highway' | 'liquidmetal' | 'neon' | 'imagegrid' | 'sculpture' | 'binary' | 'rippletank' | 'cymatics' | 'attractor' | 'stringart' | 'constellation' | 'waterfall' | 'weave' | 'synthwave' | 'bloom' | 'hive' | 'marbling' | 'flowfield' | 'truchet' | 'topography' | 'interference' | 'voronoi' | 'blobs' | 'grayscott' | 'growth' | 'pixelsort' | 'echoes' | 'physarum' | 'geodesic' | 'ribbons' | 'infinitynet' | 'arabesque' | 'murmuration' | 'epicycles' | 'knots' | 'penrose' | 'flame' | 'disorders' | 'blackwave' | 'origami' | 'lightfield' | 'brush' | 'aurora' | 'glitch' | 'warp' | 'vortex' | 'lumia' | 'woodmirror' | 'moire' | 'noctiluca' | 'tesseract' | 'supershapes' | 'corridor' | 'riemann' | 'glyphs' | 'prism' | 'cloth' | 'veil' | 'suminagashi' | 'isometric' | 'pursuit' | 'arboreal' | 'etching' | 'kintsugi' | 'klimt' | 'rosette' | 'feedbackloop' | 'kandinsky' | 'cobweb' | 'frost' | 'boogie' | 'patchwork' | 'risograph' | 'lensflare' | 'radiolaria' | 'stitch' | 'magnetosphere' | 'substrate' | 'ferrofluid' | 'cataract' | 'homage' | 'tracery' | 'protractor' | 'phyllotaxis' | 'aether' | 'chromatic' | 'particlelife' | 'tableau' | 'rorschach' | 'pollock' | 'julia' | 'langton' | 'anemone' | 'meander' | 'aggregation' | 'hopf' | 'reticulae' | 'solidlight' | 'crystaluniverse' | 'skyspace' | 'torusknot' | 'cloister' | 'melt' | 'shard' | 'stelae';

// Frequency band definitions
export interface FrequencyBand {
  name: string;
  loHz: number;
  hiHz: number;
  scale: number;
  sens: string;
  sliderId: string;
  attack: number;
  release: number;
  defaultSens: number;
}

// Octave definitions for tunnel visualization
export interface Octave {
  loHz: number;
  hiHz: number;
}

// Configuration object
export interface Config {
  // Freq mode sensitivities
  sensSub: number;
  sensBass: number;
  sensLowMid: number;
  sensMid: number;
  sensUpperMid: number;
  sensPresence: number;
  sensBrilliance: number;
  // Shared
  spikeScale: number;
  rotationSpeed: number;
  masterVolume: number;
  decayRate: number;
  intensity: number;
  beatDivision: number;
  // Highway-specific
  highwayCamFollow: number;
  highwayHorizon: number;
  highwayRoadWidth: number;
  highwayDaySpeed: number;
  // Sculpture-specific
  sculptureZoom: number;
  // Circle-specific
  circleImageRotation: number;
  // Rippletank-specific
  rippletankBeatFreq: number;
  rippletankWaterSpeed: number;
  rippletankBeatSurge: number;
  // Cymatics-specific
  cymaticsBeatFreq: number;
  cymaticsSandSize: number;
  cymaticsSandSpeed: number;
  // Attractor-specific
  attractorChaos: number;
  attractorTrailLength: number;
  // String Art-specific
  stringartPins: number;
  stringartMultiplier: number;
  stringartSpeed: number;
  // Constellation-specific
  constellationStarCount: number;
  constellationConnRange: number;
  constellationDriftSpeed: number;
  // Waterfall-specific
  waterfallScrollSpeed: number;
  waterfallGain: number;
  waterfallHue: number;
  // Weave-specific
  weaveThreads: number;
  weaveGlow: number;
  weavePulse: number;
  // Synthwave-specific
  synthwaveSpeed: number;
  synthwaveHorizon: number;
  synthwaveGlow: number;
  // Bloom-specific
  bloomDensity: number;
  bloomLifespan: number;
  bloomSpread: number;
  // Hive-specific
  hiveHexSize: number;
  hiveGlow: number;
  hiveRipple: number;
  // Marbling-specific
  marblingHue: number;
  marblingZoom: number;
  marblingSpeed: number;
  // Flow Field-specific
  flowfieldTurbulence: number;
  flowfieldTrail: number;
  flowfieldWidth: number;
  // Truchet-specific
  truchetGrid: number;
  truchetSpeed: number;
  truchetGlow: number;
  // Topography-specific
  topographyResolution: number;
  topographyLevels: number;
  topographySpeed: number;
  // Interference-specific
  interferenceFrequency: number;
  interferenceTwist: number;
  interferenceDrift: number;
  // Voronoi Stained Glass-specific
  voronoiCells: number;
  voronoiGlow: number;
  voronoiShatter: number;
  // Blobs-specific
  blobsViscosity: number;
  blobsDrift: number;
  blobsGlow: number;
  // Gray-Scott-specific
  grayscottFeed: number;
  grayscottKill: number;
  grayscottSpeed: number;
  // Growth-specific
  growthSpeed: number;
  growthTension: number;
  growthRepulsion: number;
  // Pixel Sort-specific
  pixelsortThreshold: number;
  pixelsortSpan: number;
  pixelsortHue: number;
  // Echoes-specific
  echoesDepth: number;
  echoesTwist: number;
  echoesScale: number;
  // Physarum-specific
  physarumAgents: number;
  physarumEvaporation: number;
  physarumSensor: number;
  // Geodesic-specific
  geodesicShells: number;
  geodesicSpin: number;
  geodesicGlow: number;
  // Ribbons-specific
  ribbonsCount: number;
  ribbonsWave: number;
  ribbonsShimmer: number;
  // Infinity Net-specific
  infinitynetScale: number;
  infinitynetBreathe: number;
  infinitynetPalette: number;
  // Arabesque-specific
  arabesqueSteps: number;
  arabesqueSpeed: number;
  arabesqueTrail: number;
  // Murmuration-specific
  murmuBirds: number;
  murmuCohesion: number;
  murmuTrail: number;
  // Epicycles-specific
  epicyclesCycles: number;
  epicyclesSpeed: number;
  epicyclesTrail: number;
  // Knot-specific
  knotsBeatFreq: number;
  knotsGlow: number;
  knotsSpeed: number;
  // Penrose-specific
  penroseDensity: number;
  penroseSpin: number;
  penroseGlow: number;
  // Fractal Flame-specific
  flameDensity: number;
  flameGlow: number;
  flameMutation: number;
  // Disorders-specific
  disordersGrid: number;
  disordersChaos: number;
  disordersInterrupt: number;
  // Black Wave-specific
  blackwaveDensity: number;
  blackwaveSwell: number;
  blackwaveHue: number;
  // Origami-specific
  origamiFold: number;
  origamiGrid: number;
  origamiWave: number;
  // Light Field-specific
  lightfieldGrid: number;
  lightfieldFlow: number;
  lightfieldGlow: number;
  // Brush-specific
  brushStrokes: number;
  brushWeight: number;
  brushTrail: number;
  // Aurora-specific
  auroraFlow: number;
  auroraLayers: number;
  auroraGlow: number;
  // Glitch-specific
  glitchDistort: number;
  glitchSplit: number;
  glitchNoise: number;
  // Warp-specific
  warpSpeed: number;
  warpDensity: number;
  warpTrail: number;
  // Vortex-specific
  vortexArms: number;
  vortexTwist: number;
  vortexSpeed: number;
  // Lumia-specific
  lumiaForms: number;
  lumiaDrift: number;
  lumiaGlow: number;
  // WoodMirror-specific
  woodmirrorDensity: number;
  woodmirrorDepth: number;
  woodmirrorSpeed: number;
  // Moire-specific
  moireRings: number;
  moireInterference: number;
  moireContrast: number;
  // Noctiluca-specific
  noctilucaDrift: number;
  noctilucaBloom: number;
  noctilucaWake: number;
  // Tesseract-specific
  tesseractLayers: number;
  tesseractSpin: number;
  tesseractGlow: number;
  // Super Forms-specific
  supershapesSymmetry: number;
  supershapesMorph:    number;
  supershapesGlow:     number;
  // Corridor-specific
  corridorSpeed:   number;
  corridorDepth:   number;
  corridorPalette: number;
  // Riemann Sphere-specific
  riemannSpin: number;
  riemannTilt: number;
  riemannGlow: number;
  // Glyphs-specific
  glyphsDensity: number;
  glyphsScale: number;
  glyphsDrift: number;
  // Prism-specific
  prismFilm: number;
  prismFlow: number;
  prismShimmer: number;
  // Cloth/Tapestry-specific
  clothDrape: number;
  clothRipple: number;
  clothShimmer: number;
  // Veil-specific
  veilThreads: number;
  veilOrder: number;
  veilTrail: number;
  // Suminagashi-specific
  suminagashiRings: number;
  suminagashiDrift: number;
  suminagashiBloom: number;
  // Isometric City-specific
  isometricDensity: number;
  isometricHeight: number;
  isometricPalette: number;
  // Pursuit-specific
  pursuitSymmetry: number;
  pursuitSpeed: number;
  pursuitTrail: number;
  // Arboreal-specific
  arbDepth: number;
  arbSpread: number;
  arbGlow: number;
  // Etching-specific
  etchingLines: number;
  etchingWave: number;
  etchingGlow: number;
  // Kintsugi-specific
  kintsugiFragility: number;
  kintsugiGold: number;
  kintsugiTrace: number;
  // Klimt-specific
  klimtTileSize: number;
  klimtGold: number;
  klimtComplexity: number;
  // Rosette-specific
  rosettePetals: number;
  rosetteBloom: number;
  rosetteGlow: number;
  // Feedback Loop-specific
  feedbackZoom: number;
  feedbackSpiral: number;
  feedbackGlow: number;
  // Kandinsky-specific
  kandinskyForms: number;
  kandinskyChaos: number;
  kandinskyGlow: number;
  // Cobweb-specific
  cobwebDensity: number;
  cobwebDew: number;
  cobwebTension: number;
  // Frost-specific
  frostSymmetry: number;
  frostGrowth: number;
  frostGlow: number;
  // Boogie Woogie-specific
  boogieGrid: number;
  boogiePulse: number;
  boogiePalette: number;
  // Patchwork-specific
  patchworkGrid: number;
  patchworkPalette: number;
  patchworkMotion: number;
  // Risograph-specific
  risographShapes: number;
  risographShift: number;
  risographBloom: number;
  // Lens Flare-specific
  lensflareStreak: number;
  lensflareGhosts: number;
  lensflareBloom: number;
  // Radiolaria-specific
  radiolariaSymmetry: number;
  radiolariaShells: number;
  radiolariaGlow: number;
  // Stitch-specific
  stitchGrid: number;
  stitchTrail: number;
  stitchGlow: number;

  // Magnetosphere-specific
  magnetosphereParticles: number;
  magnetosphereGravity: number;
  magnetosphereTrail: number;
  // Substrate-specific
  substrateDensity: number;
  substrateSpeed: number;
  substrateGlow: number;
  // Ferrofluid
  ferrofluidDensity: number;
  ferrofluidHeight: number;
  ferrofluidIridescence: number;
  // Cataract-specific
  cataractLines: number;
  cataractWave: number;
  cataractPalette: number;
  // Homage-specific
  homageGrid: number;
  homageNesting: number;
  homagePalette: number;
  // Tracery-specific
  traceryPetals: number;
  traceryGlow: number;
  tracerySpin: number;
  // Protractor-specific
  protractorFans: number;
  protractorRings: number;
  protractorSpread: number;
  // Phyllotaxis-specific
  phyllotaxisSeeds: number;
  phyllotaxisSpin: number;
  phyllotaxisBloom: number;
  // Aether-specific
  aetherTurbulence: number;
  aetherFlow: number;
  aetherPalette: number;
  // Chromatic-specific
  chromaticDensity: number;
  chromaticSpin: number;
  chromaticShimmer: number;
  // Particle Life-specific
  particlelifePopulation: number;
  particlelifeForce: number;
  particlelifeRange: number;
  // Tableau-specific
  tableauCount: number;
  tableauMorph: number;
  tableauFlow: number;
  // Rorschach
  rorschachDrip: number;
  rorschachSpread: number;
  rorschachPalette: number;
  // Pollock-specific
  pollockViscosity: number;
  pollockChaos: number;
  pollockPalette: number;
  // Julia-specific
  juliaDetail: number;
  juliaZoom: number;
  juliaMorph: number;
  // Langton's Ant-specific
  langtonSpeed: number;
  langtonChaos: number;
  langtonTrail: number;
  anemoneArms: number;
  anemoneWave: number;
  anemoneGlow: number;
  // Meander-specific
  meanderScatter: number;
  meanderDensity: number;
  meanderFlow: number;
  // Aggregation-specific
  aggregationGrowth: number;
  aggregationDecay: number;
  aggregationGlow: number;
  // Hopf Fibers-specific
  hopfFibers: number;
  hopfSpin: number;
  hopfGlow: number;
  // Reticulae-specific
  reticulaeDensity: number;
  reticulaeVibration: number;
  reticulaeGlow: number;
  // Solid Light-specific
  solidlightBeams: number;
  solidlightHaze: number;
  solidlightDrift: number;
  // Crystal Universe-specific
  crystaluniverseGlow: number;
  crystaluniverseFlow: number;
  crystaluniverseDrift: number;
  // Skyspace-specific
  skyspaceAperture: number;
  skyspaceWarmth: number;
  skyspaceDrift: number;
  // Torus Knot-specific
  torusKnotForm: number;
  torusKnotBore: number;
  torusKnotSpeed: number;
  // Cloister-specific
  cloisterFormation: number;
  cloisterHeight: number;
  cloisterOrbit: number;
  // Melt-specific
  meltMerge: number;
  meltDrift: number;
  meltWarmth: number;
  // Shard-specific
  shardFacets: number;
  shardScatter: number;
  shardClarity: number;
  stelaeGrid: number;
  stelaeHeight: number;
  stelaeWarmth: number;
}

// Interaction event dispatched to visualizations in interactive mode
export interface InteractionEvent {
  type: 'tap' | 'dragstart' | 'drag' | 'dragend' | 'hold' | 'key';
  /** Normalized [0,1] canvas X position */
  x: number;
  /** Normalized [0,1] canvas Y position */
  y: number;
  /** Normalized drag delta X since last event */
  dx?: number;
  /** Normalized drag delta Y since last event */
  dy?: number;
  /** Key character (keyboard events only) */
  key?: string;
}

// Application state
export interface AppState {
  mode: AnalysisMode;
  vizMode: VizMode;
  audioReady: boolean;
  isPlaying: boolean;
  useSample: boolean;
  userFile: File | null;
  currentObjectUrl: string | null;
  currentSampleBlobUrl: string | null;

  // Playback timing
  playStartedAt: number;
  startOffset: number;
  isSeeking: boolean;

  // Beat detection
  circleOutlineHue: number;
  detectedBPM: number;
  beatIntervalSec: number;
  lastBeatIndex: number;
  beatOffset: number;

}

// Wormhole visualization types
export interface WormholeEvent {
  time: number;
  band: number;
  magnitude: number;
  /** Pre-assigned value in [0, 1) used for deterministic density filtering by intensity. */
  spawnSeed: number;
}

export interface ActiveObject {
  band: number;
  hitTime: number;
  z: number;
  worldX: number;
  worldY: number;
  magnitude: number;
  hitFlash: number;
  expired: boolean;
}

// Audio processing state
export interface AudioProcessingState {
  // Smoothed band data
  smoothedBands: Float32Array[];
  transientValues: Float32Array;
  deltaValues: Float32Array;

  // Auto-gain trackers
  autoGainBands: AutoGainTracker[];

  // Transient detection
  transientBands: TransientState[];

  // Delta (rate of change) detection
  deltaBands: DeltaState[];

  // Waveform time-domain data
  waveformData: Float32Array;

  // Spectral centroid
  smoothedCentroid: number;
  centroidYOffset: number;

  // Octave-based state (tunnel mode)
  smoothedOctaves: Float32Array;
  octaveTransients: TransientState[];
  octaveTransientValues: Float32Array;
  octaveDeltas: DeltaState[];
  octaveDeltaValues: Float32Array;
  autoGainOctaves: AutoGainTracker;
}

export interface AutoGainTracker {
  peaks: Float32Array;
  idx: number;
}

export interface TransientState {
  avg: number;
  multiplier: number;
}

export interface DeltaState {
  prevMean: number;
  smoothed: number;
}

// Error types
export class AudioInitError extends Error {
  constructor(
    message: string,
    public originalError?: Error
  ) {
    super(message);
    this.name = 'AudioInitError';
  }
}

export class BPMDetectionError extends Error {
  constructor(
    message: string,
    public originalError?: Error
  ) {
    super(message);
    this.name = 'BPMDetectionError';
  }
}

// BPM detection result
export interface BPMData {
  bpm: number;
  beatOffset: number;
}

// MIDI mapping types
export interface MidiMapping {
  channel: number; // 1–16
  cc: number;      // 0–127
}
export type MidiMappings = Partial<Record<keyof Config, MidiMapping>>;
