...
│   ├── knots.ts               # Knot: torus knot T(2, q) rendered as 3 interleaved neon strands in pure p5.js 2D perspective (no Three.js); Topology slider sweeps q from trefoil (3) to septafoil (7); each strand colored by a freq register (sub+bass→violet, mid→teal, high→amber) with 60° hue gradient; 3-pass phosphor glow; beat fires angular impulse + hue shift; amplitude inflates torus radii; inspired by George W. Hart's mathematical sculpture at the Bridges Conference; sliders: Topology (q winding number 3–7), Glow (phosphor intensity), Speed (orbit rate)
...
│   ├── aurora.ts              # Aurora: 7 translucent northern-lights curtain-bands span the full canvas width (one per freq band, sub-bass at bottom → brilliance at top); each band is a sinusoidally-waving polygon filled with a luminous vertical gradient; screen-blending lets overlapping bands meld into white at full amplitude; stars drift in a deep-navy sky; beat fires a cool-blue brightness surge and rotates the hue palette; inspired by James Turrell "Aurora B: Tall Glass" (2010); sliders: Intensity (brightness 0–2), Turbulence (wave harmonic count), Drift (animation speed)
│   ├── nebula.ts              # Nebula: cosmic gas cloud visualization inspired by Refik Anadol's "Machine Memoirs: Space" (2020, https://refikanadol.com/works/machine-memoirs-space/); 7 gas cloud sources (one per freq band) orbit a luminous core; each cloud's brightness and radius pulse with band amplitude; additive pixel-buffer blending creates ethereal color mixing; wispy sin-perturbation gives organic edges; twinkling star field with audio-correlated brightness; beat fires expanding light ring from core + hue palette shift; offscreen pixel buffer at ¼ res (⅙ mobile); sliders: Density (gas opacity), Drift (cosmic wind speed), Stars (star brightness/count)
...
│   └── keyboard.ts            # Keyboard shortcuts (0-9 viz modes, letter shortcuts, space, arrows, m/f/s/r/i/?/h/Esc; shortcuts auto-registered from VIZ_REGISTRY; special keys: ;→marbling, [→flowfield, ]→lissajous, \→truchet, -→topography, =→interference, .→voronoi, ,→blobs, /→grayscott, ~→growth, @→pixelsort, #→echoes, $→physarum, %→optical, ^→geodesic, &→ribbons, *→infinitynet, _→zengarden, !→arabesque, |→murmuration, q→blackwave, l→flame, k→disorders, a→aurora, c→nebula, d→brush)
├── types/
