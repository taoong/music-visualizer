...
│   ├── lattice.ts             # Lattice: rectangular grid of vertices elastically deformed by audio; each row-stripe maps to a freq band (sub-bass=top → brilliance=bottom) driving sinusoidal Y-displacement; column bands drive X; Perlin noise adds organic drift; beat fires a radial ripple wave; 3-pass neon glow with per-band jewel-tone hues (violet→magenta); sliders: Grid (5–24 columns), Warp (displacement amplitude), Glow (neon intensity)
