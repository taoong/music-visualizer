...
│   ├── aurora.ts              # Aurora: 7 vertical light curtains span the full canvas; each curtain driven by a freq band (sub-bass=left → brilliance=right); Perlin noise sways each curtain laterally and wrinkles its edges; vertical gradient creates an aurora crown — bright at top (~10–18 %), fading to void at bottom; screen-blend compositing lets overlapping curtains accumulate brightness like real aurora bands; beats fire a whole-sky pulse; faint star field in background; sliders: Curtains (1–7 bands), Wave (turbulence 0=glassy→1=storm), Hue (0=emerald green→0.5=sapphire→1=crimson violet)
...
│   ├── datadream.ts           # Data Dream: luminous point-cloud "data fog" inspired by Refik Anadol's Machine Hallucinations / Dataland (opens June 20, 2026, https://refikanadol.com/); thousands of glowing particles drift through a Perlin flow field, each tinted by one of 7 freq bands (cool data-blue sub-bass → indigo → violet → magenta → warm gold brilliance); beats trigger a "dream" — particles are pulled into a freshly generated organic polar-curve silhouette ("data sculpture"), hold, then dissolve back into fog; offscreen ADD-blend glow buffer; sliders: Density (particle count), Flow (field speed/turbulence), Form (strength/persistence of beat-triggered dream formation)
...
│   └── keyboard.ts            # Keyboard shortcuts (0-9 viz modes, letter shortcuts, space, arrows, m/f/s/r/i/?/h/Esc; shortcuts auto-registered from VIZ_REGISTRY; special keys: ;→marbling, [→flowfield, ]→lissajous, \→truchet, -→topography, =→interference, .→voronoi, ,→blobs, /→grayscott, ~→growth, @→pixelsort, #→echoes, $→physarum, %→optical, ^→geodesic, &→ribbons, *→infinitynet, _→zengarden, !→arabesque, |→murmuration, q→blackwave, l→flame, a→aurora, k→disorders)
├── types/
