...
│   ├── noodles.ts             # Noodles: 7 glowing neon serpents (one per freq band) swim freely across the canvas; each noodle is a ring-buffered chain of positions — head steered by Perlin noise scaled by audio amplitude, body follows as a trail; 3-pass glow (outer/mid/core) with quadratic alpha fade from tail to head; beat surge boosts speed and brightness; soft boundary repulsion keeps noodles on screen; sliders: Length (body segment count), Speed, Thickness
...
│   ├── stainedglass.ts        # Stained Glass: canvas recursively BSP-subdivided into irregular convex polygons; each polygon maps to one of 7 freq bands (assigned by angle from canvas centre); fill color = HSB with band-driven saturation/brightness; two-pass glowing white edges simulate lead lines (ADD blend); beat triggers full-canvas flash; hue drifts slowly for ambient life; sliders: Shards (8–64), Edge Glow (0–5), Color Drift (0–3)
