...
│   ├── waterfall.ts           # Waterfall: scrolling spectrogram; each frame's spectrum painted as a 1-col strip on the right edge of an offscreen canvas, buffer shifts left each frame; sustained tones = horizontal streaks, transients = vertical bursts; plasma palette via 256-entry RGB LUT; beat tick overlays a faint white line on the newest column; sliders: Scroll Speed, Brightness, Palette Hue
