...
| `a` | Aurora | Seven translucent northern-lights curtain-bands sweep the full canvas width, one per frequency band; sub-bass hangs at the bottom, brilliance crowns the top; each curtain is a sinusoidally-waving polygon filled with a luminous vertical gradient (deep-violet → cobalt → teal → emerald → lime → rose → ice-blue); screen-blending lets overlapping bands meld into white where multiple frequencies peak simultaneously; twinkling stars drift in a deep-navy sky; beats fire a cool-blue brightness surge and rotate the hue palette; inspired by James Turrell "Aurora B: Tall Glass" (2010) and his Skyspace installations where layered LED light achieves an almost tactile luminosity — https://turrell.utexas.edu/ — sliders: Intensity (overall aurora brightness, 0 = barely visible → 2 = blazing), Turbulence (wave harmonic complexity, smooth ripple → chaotic multi-harmonic curtains), Drift (animation speed, nearly still → fast flowing) |
| `c` | Nebula | Cosmic gas cloud visualization inspired by Refik Anadol's "Machine Memoirs: Space" (2020) — https://refikanadol.com/works/machine-memoirs-space/ — where millions of NASA satellite and telescope images are fed through ML algorithms to produce swirling, luminous cosmic landscapes; 7 gas cloud sources (one per frequency band) orbit a pulsing luminous core, each cloud's brightness and radius driven by its band's amplitude; additive blending in an offscreen pixel buffer (¼ res, ⅙ mobile) creates ethereal color mixing where clouds overlap; wispy sin-based perturbation gives organic, tendril-like edges; a twinkling star field drifts behind the gas with audio-correlated brightness; beats fire an expanding light ring from the core and shift the cosmic hue palette; sliders: Density (gas cloud opacity, faint wisps → thick luminous clouds), Drift (cosmic wind speed, near-still → fast swirling), Stars (star brightness and density, empty sky → dense twinkling field) |
